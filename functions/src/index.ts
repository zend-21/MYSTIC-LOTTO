...
export const getFortuneAndNumbers = onCall(
  ...
  `;
    ...
    if (!text) {
      console.error("[getFortuneAndNumbers] response.text is null. finishReason:", response.candidates?.[0]?.finishReason);
...
export const getFixedDestinyNumbers = onCall(
  ...
  async (request) => {
    ...
    const lnyStr = LUNAR_NEW_YEAR[currentYear] ?? `${currentYear}-02-05`;
  ...
  `;
      ...
      config: {
        ...
        responseSchema: {
          ...
          properties: {
            ...
            luckyColorDescription: { type: Type.STRING },
    ...
    const text = response.text;
    if (!text) {
      console.error("[getFixedDestinyNumbers] response.text is null. finishReason:", response.candidates?.[0]?.finishReason, "safetyRatings:", JSON.stringify(response.candidates?.[0]?.safetyRatings));
      throw new HttpsError("internal", "AI 응답이 비어있습니다.");
    }
    let result: any;
    try {
      ...
      console.error("[getFixedDestinyNumbers] JSON.parse failed. raw:", text?.slice(0, 300));
    ...
    const annual = {
      ...
      luckyColorDescription: result.luckyColorDescription ?? "",
...
// 봉헌 수행 (확률 계산 서버사이드)
// ──────────────────────────────────────────────
export const performOffering = onCall(
  { region: "asia-northeast3" },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const uid = request.auth.uid;
    ...
    if (!amount || typeof amount !== "number" || amount <= 0 || !Number.isInteger(amount)) {
      throw new HttpsError("invalid-argument", "유효하지 않은 봉헌 금액입니다.");
    }

    // 사용자 레벨 조회
    const userSnap = await db.collection("users").doc(uid).get();
    if (!userSnap.exists) throw new HttpsError("not-found", "사용자 데이터를 찾을 수 없습니다.");
    const level: number = userSnap.data()?.orb?.level ?? 1;
    ...
    if (level >= 100) {
      ...
      if (rand < 0.20) multiplier = 10;
      else if (rand < 0.55) multiplier = 5;
      else multiplier = 2;
    } else if (level >= 80) {
      // 초월 황금태양: 1x:15%, 2x:40%, 5x:30%, 10x:15%
      if (rand < 0.15) multiplier = 10;
      else if (rand < 0.45) multiplier = 5;
      else if (rand < 0.85) multiplier = 2;
      else multiplier = 1;
    } else if (level >= 50) {
      // 성운의 정수: 1x:30%, 2x:35%, 5x:25%, 10x:10%
      if (rand < 0.10) multiplier = 10;
      else if (rand < 0.35) multiplier = 5;
      else if (rand < 0.70) multiplier = 2;
      else multiplier = 1;
    } else if (level >= 10) {
      // 각성의 구슬: 1x:40%, 2x:33%, 5x:20%, 10x:7%
      if (rand < 0.07) multiplier = 10;
      else if (rand < 0.27) multiplier = 5;
      else if (rand < 0.60) multiplier = 2;
      else multiplier = 1;
    } else {
      // 태동의 씨앗: 1x:50%, 2x:30%, 5x:15%, 10x:5%
      if (rand < 0.05) multiplier = 10;
      else if (rand < 0.20) multiplier = 5;
      else if (rand < 0.50) multiplier = 2;
      else multiplier = 1;
    }
...
export const claimDailyBonus = onCall(
  ...
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const uid = request.auth.uid;
    if (uid === ADMIN_UID) return { granted: false };
    ...
    return db.runTransaction(async (tx) => {
      ...
      if ((snap.data()?.orb?.lastVisitDate ?? "") === today) return { granted: false };
      tx.update(userRef, {
...
// ──────────────────────────────────────────────
export const processInbox = onCall(
  { region: "asia-northeast3" },
  async (request) => {
    if (!request.auth) throw new HttpsError("unauthenticated", "로그인이 필요합니다.");
    const uid = request.auth.uid;
    ...
    if (inboxSnap.empty) return { processed: 0, totalGift: 0, totalExp: 0, senders: [] as string[] };

    let totalGift = 0;
    let totalExp = 0;
    const senders: string[] = [];
    const giftEntries: object[] = [];

    inboxSnap.docs.forEach((d) => {
      const data = d.data();
      if (data.type === "exp") {
        totalExp += (data.amount as number) || 0;
      } else {
        const amt = (data.amount as number) || 0;
        totalGift += amt;
        if (data.fromName) senders.push(data.fromName as string);
        giftEntries.push({
          id: d.id,
          type: "received",
          targetName: (data.fromName as string) || "알 수 없음",
          amount: amt,
